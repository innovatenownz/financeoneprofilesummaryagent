...
# --- KNOWLEDGE BASE: INSURANCE CLAIMS SCHEMA ---
INSURANCE_CLAIMS_SCHEMA = [
    {"api": "id", "type": "text"},
    {"api": "Name", "type": "text"}, # Claim Name
    ...
    # Claim Details
    {"api": "Claim_Type", "type": "picklist"}, # e.g. Accident, Theft
    {"api": "Status", "type": "picklist"}, # e.g. Submitted, Settled
    {"api": "Date_of_Loss", "type": "date"},
    {"api": "Amount_Claimed", "type": "currency"},
    {"api": "Amount_Paid", "type": "currency"},
    {"api": "Settlement_Date", "type": "date"},
    ...
    {"api": "Claimant", "type": "lookup", "target": "Contacts"},
...
class InsuranceClaimsNewAgent:
    def __init__(self, model):
        self.model = model
        self.schema_string = "\n".join([f"- {f['api']} ({f['type']})" for f in INSURANCE_CLAIMS_SCHEMA])

    def format_data_for_ai(self, record: dict) -> str:
        """
        Parses Insurance Claim JSON into a clean, readable text block.
        """
        if not record: return "No Insurance Claim Data Available."

        # === 1. LIST MODE (Context Switch / Search Results) ===
        if "items" in record:
            items = record["items"]
            if not items: return "No Insurance Claims found."
            
            lines = [f"=== FOUND {len(items)} CLAIMS ==="]
            for i, item in enumerate(items, 1):
                ...
                critical_fields = ["Status", "Amount_Claimed", "Date_of_Loss", "Insurance_Policy"]
        ...
        lines = ["=== INSURANCE CLAIM DETAILS ==="]

        # Process Standard Fields
        for field in INSURANCE_CLAIMS_SCHEMA:
            key = field['api']
            val = record.get(key)
    ...
    def generate_response(self, user_query: str, claim_data: dict, history: list = []) -> str:
        """
        Generates a response answering questions about the Insurance Claim.
        """
        context_text = self.format_data_for_ai(claim_data)
        ...
        prompt = f"""
        You are an expert Insurance Claims Specialist.
        ...
        - **Status:** Check 'Status' and 'Settlement Date' to see if the claim is closed.
        - **Financials:** Compare 'Amount Claimed' vs 'Amount Paid'.
        - **Context:** Identify the linked 'Insurance Policy' and 'Claimant'.

        ### ACTION PROTOCOL
        <<<ACTION>>>
        {{
            "action": "create" | "update",
            "module": "Insurance_Claims_New", 
            "record_id": "12345",
            "data": {{ 
                "Name": "REQUIRED_CLAIM_NAME",
                "Status": "Submitted",
                "Field_Name": "Value" 
            }}
        }}
