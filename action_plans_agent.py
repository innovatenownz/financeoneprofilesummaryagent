...
# --- KNOWLEDGE BASE: ACTION PLANS SCHEMA ---
ACTION_PLANS_SCHEMA = [
    {"api": "id", "type": "text"},
    {"api": "Name", "type": "text"}, # Plan Name
    ...
    # Plan Details
    {"api": "Plan_Type", "type": "picklist"},
    {"api": "Status", "type": "picklist"},
    {"api": "Executive_Summary", "type": "textarea"},
    {"api": "Recommendations", "type": "textarea"},
    {"api": "Locked__s", "type": "boolean"},
    
    # Relationships (Lookups with Targets)
    {"api": "Fact_Find", "type": "lookup", "target": "Fact_Find_New"},
    {"api": "Deals", "type": "lookup", "target": "Deals"},
    
    # Advisory Team
    {"api": "Action_Plan_Owners", "type": "multiuserlookup", "target": "Users"}
]

class ActionPlansAgent:
    def __init__(self, model):
        self.model = model
        self.schema_string = "\n".join([f"- {f['api']} ({f['type']})" for f in ACTION_PLANS_SCHEMA])

    def format_data_for_ai(self, record: dict) -> str:
        """
        Parses Action Plan JSON into a clean, readable text block.
        """
        if not record: return "No Action Plan Data Available."

        # === 1. LIST MODE (Context Switch / Search Results) ===
        if "items" in record:
            items = record["items"]
            if not items: return "No Action Plans found."
            
            lines = [f"=== FOUND {len(items)} ACTION PLANS ==="]
            for i, item in enumerate(items, 1):
                ...
                critical_fields = ["Status", "Plan_Type", "Fact_Find", "Deals"]
        ...
        lines = ["=== ACTION PLAN DETAILS ==="]

        # Process Standard Fields
        for field in ACTION_PLANS_SCHEMA:
            key = field['api']
            val = record.get(key)
            ...
            if key == "Action_Plan_Owners" and isinstance(val, list):
    ...
    def generate_response(self, user_query: str, plan_data: dict, history: list = []) -> str:
        """
        Generates a response answering questions about the Action Plan.
        """
        context_text = self.format_data_for_ai(plan_data)
        ...
        - **Summary:** Summarize 'Executive Summary' and 'Recommendations'.
        - **Status:** Check 'Status' (e.g., Draft, In Progress).

        ### ACTION PROTOCOL
        <<<ACTION>>>
        {{
            "action": "create" | "update",
            "module": "Action_Plans_New", 
            "record_id": "12345",
            "data": {{ 
                "Name": "REQUIRED_NAME",
                "Status": "Draft",
