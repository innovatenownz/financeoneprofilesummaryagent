...
# --- KNOWLEDGE BASE: INSURANCE POLICIES SCHEMA ---
INSURANCE_POLICIES_SCHEMA = [
    {"api": "id", "type": "text"},
    {"api": "Name", "type": "text"}, # Policy Number/Name
    ...
    # Policy Details
    {"api": "Policy_Type", "type": "picklist"}, # e.g. Life Insurance, TPD
    {"api": "Status", "type": "picklist"}, # e.g. Active, In Application
    {"api": "Coverage_Amount", "type": "currency"},
    {"api": "Premium", "type": "currency"},
    {"api": "Premium_Frequency", "type": "picklist"}, # e.g. Monthly, Annually
    {"api": "Policy_Start_Date", "type": "date"},
    {"api": "Renewal_Date", "type": "date"},
    {"api": "Next_Renewal_Date", "type": "formula"},
    {"api": "Locked__s", "type": "boolean"},
    
    # Relationships (Lookups with Targets)
    {"api": "Insurance_Profile", "type": "lookup", "target": "Insurance_Profiles_New"},
    {"api": "Insurer", "type": "lookup", "target": "Vendors"},
...
class InsurancePoliciesNewAgent:
    def __init__(self, model):
        self.model = model
        self.schema_string = "\n".join([f"- {f['api']} ({f['type']})" for f in INSURANCE_POLICIES_SCHEMA])

    def format_data_for_ai(self, record: dict) -> str:
        """
        Parses Insurance Policy JSON into a clean, readable text block.
        """
        if not record: return "No Insurance Policy Data Available."

        # === 1. LIST MODE (Context Switch / Search Results) ===
        if "items" in record:
            items = record["items"]
            if not items: return "No Insurance Policies found."
            ...
            for i, item in enumerate(items, 1):
                ...
                if "Name" in item: lines.append(f"Policy #: {item['Name']}")
                
                # B. FORCE SHOW critical fields
                critical_fields = ["Policy_Type", "Status", "Premium", "Insurance_Profile"]
        ...
        lines = ["=== INSURANCE POLICY DETAILS ==="]

        # Process Standard Fields
        for field in INSURANCE_POLICIES_SCHEMA:
            key = field['api']
            val = record.get(key)
    ...
    def generate_response(self, user_query: str, policy_data: dict, history: list = []) -> str:
        """
        Generates a response answering questions about the Insurance Policy.
        """
        context_text = self.format_data_for_ai(policy_data)
        ...
        - **Coverage:** Clearly state 'Coverage Amount' and 'Premium'.
        - **Renewal:** Highligh 'Renewal Date' or 'Next Renewal Date'.
        - **Context:** Identify the 'Insurer' and linked 'Insurance Profile'.

        ### ACTION PROTOCOL
        <<<ACTION>>>
        {{
            "action": "create" | "update",
            "module": "Insurance_Policies_New", 
            "record_id": "12345",
            "data": {{ 
                "Name": "REQUIRED_POLICY_NUMBER",
