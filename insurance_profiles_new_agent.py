...
# --- KNOWLEDGE BASE: INSURANCE PROFILE NEW SCHEMA ---
INSURANCE_PROFILE_NEW_SCHEMA = [
    {"api": "id", "type": "text"},
    {"api": "Name", "type": "text"}, # Insurance Profile Name
...
class InsuranceProfileNewAgent:
    def __init__(self, model):
        self.model = model
        self.schema_string = "\n".join([f"- {f['api']} ({f['type']})" for f in INSURANCE_PROFILE_NEW_SCHEMA])

    def format_data_for_ai(self, record: dict) -> str:
        """
        Parses Insurance Profile JSON into a clean, readable text block.
        """
        if not record: return "No Insurance Profile Data Available."

        # === 1. LIST MODE (Context Switch / Search Results) ===
        if "items" in record:
            items = record["items"]
            if not items: return "No Insurance Profiles found."
            
            lines = [f"=== FOUND {len(items)} INSURANCE PROFILES ==="]
            for i, item in enumerate(items, 1):
                ...
                critical_fields = ["Primary_Advisor", "Owner"]
        ...
        lines = ["=== INSURANCE PROFILE DETAILS ==="]

        # Process Standard Fields
        for field in INSURANCE_PROFILE_NEW_SCHEMA:
            key = field['api']
            val = record.get(key)
    ...
    def generate_response(self, user_query: str, profile_data: dict, history: list = []) -> str:
        """
        Generates a response answering questions about the Insurance Profile.
        """
        ...
        prompt = f"""
        You are an expert Insurance Advisor & Policy Manager.
        ...
        - **Advisory:** Identify the 'Primary Advisor' managing this profile.
        - **Content:** This record is a summary profile. Look for related lists (like 'Insurance Policies') if detailed policy data is needed but missing here.

        ### ACTION PROTOCOL
        <<<ACTION>>>
        {{
            "action": "create" | "update",
            "module": "Insurance_Profiles_New", 
            "record_id": "12345",
            "data": {{ 
                "Name": "REQUIRED_NAME",
                "Field_Name": "Value" 
