...
# --- KNOWLEDGE BASE: INSURANCE POLICY HOLDER SCHEMA ---
INSURANCE_HOLDER_SCHEMA = [
    {"api": "id", "type": "text"},
    {"api": "Name", "type": "text"}, # Holder Name
    ...
    {"api": "City", "type": "text"},
    {"api": "State", "type": "text"},
    {"api": "Zip_Code", "type": "integer"},
    {"api": "Country", "type": "text"},
    
    # Personal/Entity Details
    {"api": "Holder_Type", "type": "picklist"}, # Individual vs Entity
    {"api": "Date_of_Birth", "type": "date"},
    {"api": "Gender", "type": "picklist"},
    {"api": "Government_Id", "type": "text"},
    {"api": "Locked__s", "type": "boolean"},
    
    # Relationships (Lookups with Targets)
    {"api": "Insurance_Policies", "type": "lookup", "target": "Insurance_Policies_New"},
    {"api": "Insured_party", "type": "lookup", "target": "Contacts"}, # Person
    {"api": "Insured_entity", "type": "lookup", "target": "Accounts"}, # Company/Trust
...
class InsurancePolicyHolderAgent:
    def __init__(self, model):
        self.model = model
        self.schema_string = "\n".join([f"- {f['api']} ({f['type']})" for f in INSURANCE_HOLDER_SCHEMA])

    def format_data_for_ai(self, record: dict) -> str:
        """
        Parses Insurance Policy Holder JSON into a clean, readable text block.
        """
        if not record: return "No Policy Holder Data Available."

        # === 1. LIST MODE (Context Switch / Search Results) ===
        if "items" in record:
            items = record["items"]
            if not items: return "No Policy Holders found."
            
            lines = [f"=== FOUND {len(items)} HOLDERS ==="]
            for i, item in enumerate(items, 1):
                ...
                critical_fields = ["Holder_Type", "Email", "Phone", "Insurance_Policies"]
        ...
        lines = ["=== POLICY HOLDER DETAILS ==="]

        # Process Standard Fields
        for field in INSURANCE_HOLDER_SCHEMA:
            key = field['api']
            val = record.get(key)
    ...
    def generate_response(self, user_query: str, holder_data: dict, history: list = []) -> str:
        """
        Generates a response answering questions about the Insurance Policy Holder.
        """
        context_text = self.format_data_for_ai(holder_data)
        ...
        - **Identity:** Check 'Holder Type'. If 'Individual', look for 'Date of Birth' and 'Gender'. If 'Entity', look for company details.
        - **Context:** Mention the linked 'Insurance Policies' this holder belongs to.

        ### ACTION PROTOCOL
        <<<ACTION>>>
        {{
            "action": "create" | "update",
            "module": "Insurance_Policy_Holder_N", 
            "record_id": "12345",
            "data": {{ 
                "Name": "REQUIRED_NAME",
