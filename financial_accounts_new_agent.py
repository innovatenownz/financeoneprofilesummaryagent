...
# --- KNOWLEDGE BASE: FINANCIAL ACCOUNTS SCHEMA ---
FINANCIAL_ACCOUNTS_SCHEMA = [
    {"api": "id", "type": "text"},
    {"api": "Name", "type": "text"}, # Accounts Name
    ...
    # Account Details
    {"api": "Institution_Platform", "type": "text"},
    {"api": "Account_Number", "type": "text"},
    {"api": "Account_Type", "type": "picklist"}, # Investment, Superannuation, Bank Account
    {"api": "Status", "type": "picklist"}, # Open, Closed, etc.
    {"api": "Locked__s", "type": "boolean"},
    
    # Financials
    {"api": "Balance", "type": "currency"},
    {"api": "Cash_Balance", "type": "currency"},
    {"api": "Invested_Balance1", "type": "formula"}, # Currency
    
    # Relationships (Lookups with Targets)
    {"api": "Investment_holding", "type": "lookup", "target": "Investment_Holdings_New"},
    
    # Advisory Team
    {"api": "Primary_Advisor", "type": "userlookup", "target": "Users"},
    {"api": "Secondary_Advisor", "type": "userlookup", "target": "Users"},
    {"api": "Financial_Account_Team", "type": "multiuserlookup", "target": "Users"}
]

class FinancialAccountsNewAgent:
    def __init__(self, model):
        self.model = model
        self.schema_string = "\n".join([f"- {f['api']} ({f['type']})" for f in FINANCIAL_ACCOUNTS_SCHEMA])

    def format_data_for_ai(self, record: dict) -> str:
        """
        Parses Financial Account JSON into a clean, readable text block.
        """
        if not record: return "No Financial Account Data Available."

        # === 1. LIST MODE (Context Switch / Search Results) ===
        if "items" in record:
            items = record["items"]
            if not items: return "No Financial Accounts found."
            
            lines = [f"=== FOUND {len(items)} ACCOUNTS ==="]
            for i, item in enumerate(items, 1):
                ...
                critical_fields = ["Institution_Platform", "Balance", "Account_Type", "Status"]
        ...
        lines = ["=== FINANCIAL ACCOUNT DETAILS ==="]

        # Process Standard Fields
        for field in FINANCIAL_ACCOUNTS_SCHEMA:
            key = field['api']
            val = record.get(key)
            ...
            if key == "Financial_Account_Team" and isinstance(val, list):
    ...
    def generate_response(self, user_query: str, account_data: dict, history: list = []) -> str:
        """
        Generates a response answering questions about the Financial Account.
        """
        ...
        prompt = f"""
        You are an expert Financial Planner & Portfolio Administrator.
        ...
        - **Identification:** Identify the 'Institution/Platform' and 'Account Number'.
        - **Financials:** Report 'Balance' and break down 'Cash' vs 'Invested' balance if available.

        ### ACTION PROTOCOL
        <<<ACTION>>>
        {{
            "action": "create" | "update",
            "module": "Financial_Accounts_New", 
            "record_id": "12345",
            "data": {{ 
                "Name": "REQUIRED_NAME",
                "Institution_Platform": "Bank ABC",
                "Field_Name": "Value" 
            }}
        }}
        <<<END_ACTION>>>
