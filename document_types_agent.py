...
# --- KNOWLEDGE BASE: DOCUMENT TYPES SCHEMA ---
DOCUMENT_TYPES_SCHEMA = [
    {"api": "id", "type": "text"},
    {"api": "Name", "type": "autonumber"}, # Document Type Name (Auto Number)
    {"api": "Document", "type": "text"}, # Actual Name/Label of the Document Type
    ...
    {"api": "Document_Category", "type": "picklist"}, # e.g. Legal, Financial
    {"api": "Sensitivity_Level", "type": "picklist"}, # e.g. Confidential, Public
    {"api": "Retention_Period", "type": "integer"}, # Days/Months/Years (Context dependent)
    {"api": "Allowed_Formats", "type": "multiselectpicklist"}, # e.g. PDF, DOCX
    {"api": "Description_Instructions", "type": "textarea"},
    {"api": "Locked__s", "type": "boolean"}
]

class DocumentTypesAgent:
    def __init__(self, model):
        self.model = model
        self.schema_string = "\n".join([f"- {f['api']} ({f['type']})" for f in DOCUMENT_TYPES_SCHEMA])

    def format_data_for_ai(self, record: dict) -> str:
        """
        Parses Document Type JSON into a clean, readable text block.
        """
        if not record: return "No Document Type Data Available."

        # === 1. LIST MODE (Context Switch / Search Results) ===
        if "items" in record:
            items = record["items"]
            if not items: return "No Document Types found."
            
            lines = [f"=== FOUND {len(items)} DOCUMENT TYPES ==="]
            for i, item in enumerate(items, 1):
                ...
                if "Document" in item: lines.append(f"Type Name: {item['Document']}")
                
                # B. FORCE SHOW critical fields
                critical_fields = ["Document_Category", "Sensitivity_Level", "Retention_Period"]
                
                for k, v in item.items():
                    if k not in ["id", "Name", "Tag", "Document"]:
                        val_display = v
        ...
        lines = ["=== DOCUMENT TYPE DETAILS ==="]

        # Process Standard Fields
        for field in DOCUMENT_TYPES_SCHEMA:
            key = field['api']
            val = record.get(key)
            ...
            # Formatting Multi-Select
            if field['type'] == 'multiselectpicklist' and isinstance(val, list):
                val = ", ".join(val)
    ...
    def generate_response(self, user_query: str, doc_type_data: dict, history: list = []) -> str:
        """
        Generates a response answering questions about the Document Type.
        """
        context_text = self.format_data_for_ai(doc_type_data)
        ...
        - **Policy:** Explain 'Retention Period' and 'Sensitivity Level'.
        - **Formats:** List 'Allowed Formats'.
        - **Instructions:** Mention any specific 'Description/Instructions'.
