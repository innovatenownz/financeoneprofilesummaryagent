...
# --- KNOWLEDGE BASE: EXPENSES NEW SCHEMA ---
EXPENSES_NEW_SCHEMA = [
    {"api": "id", "type": "text"},
    {"api": "Name", "type": "text"}, # Expenses Name
    ...
    {"api": "Expense_Type", "type": "picklist"}, # e.g. Fixed, Variable
    {"api": "Frequency", "type": "picklist"}, # e.g. Monthly, Annually
    {"api": "Locked__s", "type": "boolean"},
    
    # Relationships (Lookups with Targets)
    {"api": "Client", "type": "lookup", "target": "Contacts"},
    {"api": "Expense_Profile", "type": "lookup", "target": "Expense_Profile_New"},
    ...
    {"api": "Expenses_Owners", "type": "multiuserlookup", "target": "Users"}
]

class ExpensesNewAgent:
    def __init__(self, model):
        self.model = model
        self.schema_string = "\n".join([f"- {f['api']} ({f['type']})" for f in EXPENSES_NEW_SCHEMA])

    def format_data_for_ai(self, record: dict) -> str:
        """
        Parses Expenses New JSON into a clean, readable text block.
        """
        if not record: return "No Expense Data Available."

        # === 1. LIST MODE (Context Switch / Search Results) ===
        if "items" in record:
            items = record["items"]
            if not items: return "No Expenses found."
            
            lines = [f"=== FOUND {len(items)} EXPENSES ==="]
            for i, item in enumerate(items, 1):
                ...
                critical_fields = ["Amount", "Payee", "Frequency", "Expense_Profile"]
        ...
        for field in EXPENSES_NEW_SCHEMA:
            key = field['api']
            val = record.get(key)
            ...
            if key == "Expenses_Owners" and isinstance(val, list):
    ...
    def generate_response(self, user_query: str, expense_data: dict, history: list = []) -> str:
        """
        Generates a response answering questions about the Expense.
        """
        ...
        prompt = f"""
        You are an expert Financial Planner & Budget Analyst.
        
        ### DATA SCHEMA
        {self.schema_string}

        ### EXPENSE RECORD CONTEXT
        {context_text}
        ...
        - **Cost:** Clearly state 'Amount' and 'Frequency'.
        - **Payee:** Identify who is being paid ('Payee').
        - **Context:** Mention the linked 'Expense Profile' or 'Client'.
