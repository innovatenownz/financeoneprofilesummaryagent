...
# --- KNOWLEDGE BASE: AML RECORDS SCHEMA ---
AML_RECORDS_SCHEMA = [
    {"api": "id", "type": "text"},
    {"api": "Name", "type": "text"}, # AML Record Name
    ...
    # Compliance Details
    {"api": "Risk_Rating", "type": "picklist"}, # High, Medium, Low
    {"api": "PEP_Status", "type": "picklist"}, # Politically Exposed Person
    {"api": "Source_of_Funds", "type": "textarea"},
    {"api": "Reason_for_Rating", "type": "textarea"},
    {"api": "Assessment_Date", "type": "date"},
    {"api": "Last_Screened_Date", "type": "date"},
    {"api": "Next_Review_Date", "type": "date"},
    {"api": "Locked__s", "type": "boolean"},
    
    # Relationships (Lookups with Targets)
    {"api": "Client", "type": "lookup", "target": "Contacts"},
    {"api": "Risks", "type": "lookup", "target": "Risk_Profile_New"}, # Links to Risk Profile
    
    # Advisory Team
    {"api": "Primary_Advisor", "type": "userlookup", "target": "Users"},
    {"api": "Secondary_Advisor", "type": "userlookup", "target": "Users"},
    {"api": "AML_Record_Owners", "type": "multiuserlookup", "target": "Users"}
]

class AMLRecordsAgent:
    def __init__(self, model):
        self.model = model
        self.schema_string = "\n".join([f"- {f['api']} ({f['type']})" for f in AML_RECORDS_SCHEMA])

    def format_data_for_ai(self, record: dict) -> str:
        """
        Parses AML Record JSON into a clean, readable text block.
        """
        if not record: return "No AML Record Data Available."

        # === 1. LIST MODE (Context Switch / Search Results) ===
        if "items" in record:
            items = record["items"]
            if not items: return "No AML Records found."
            
            lines = [f"=== FOUND {len(items)} AML RECORDS ==="]
            for i, item in enumerate(items, 1):
                ...
                critical_fields = ["Risk_Rating", "PEP_Status", "Next_Review_Date", "Client"]
        ...
        lines = ["=== AML RECORD DETAILS ==="]

        # Process Standard Fields
        for field in AML_RECORDS_SCHEMA:
            key = field['api']
            val = record.get(key)
            ...
            if key == "AML_Record_Owners" and isinstance(val, list):
    ...
    def generate_response(self, user_query: str, aml_data: dict, history: list = []) -> str:
        """
        Generates a response answering questions about the AML Record.
        """
        context_text = self.format_data_for_ai(aml_data)
        ...
        - **Risk:** Clearly state 'Risk Rating' and 'PEP Status'.
        - **Review:** Highlight 'Next Review Date' or 'Last Screened Date'.
        - **Source:** Explain 'Source of Funds' if relevant.

        ### ACTION PROTOCOL
        <<<ACTION>>>
        {{
            "action": "create" | "update",
            "module": "AML_Records_New", 
            "record_id": "12345",
            "data": {{ 
                "Name": "REQUIRED_RECORD_NAME",
                "Risk_Rating": "Low",
                "Field_Name": "Value" 
            }}
        }}
