...
# --- KNOWLEDGE BASE: LIABILITIES NEW SCHEMA ---
LIABILITIES_NEW_SCHEMA = [
    {"api": "id", "type": "text"},
    {"api": "Name", "type": "text"}, # Liability Name
    ...
    # Financial Details
    {"api": "Liability_Amount", "type": "currency"},
    {"api": "Total_Current_Loan_Balance", "type": "currency"},
    {"api": "Total_Original_Loan_Balance", "type": "currency"},
    {"api": "Laibility_Type", "type": "picklist"}, # Note: keeping API spelling
    {"api": "Ownership", "type": "picklist"},
    {"api": "Role", "type": "picklist"},
    ...
    {"api": "Loan", "type": "lookup", "target": "Loans"}, # Links to Loans Module
    {"api": "Real_Estate", "type": "lookup", "target": "Real_Estate_Assets_New"}
]

class LiabilitiesNewAgent:
    def __init__(self, model):
        self.model = model
        self.schema_string = "\n".join([f"- {f['api']} ({f['type']})" for f in LIABILITIES_NEW_SCHEMA])

    def format_data_for_ai(self, record: dict) -> str:
        """
        Parses Liability JSON into a clean, readable text block.
        """
        if not record: return "No Liability Data Available."

        # === 1. LIST MODE (Context Switch / Search Results) ===
        if "items" in record:
            items = record["items"]
            if not items: return "No Liabilities found."
            
            lines = [f"=== FOUND {len(items)} LIABILITIES ==="]
            for i, item in enumerate(items, 1):
                ...
                critical_fields = ["Liability_Amount", "Laibility_Type", "Total_Current_Loan_Balance", "Client"]
        ...
        lines = ["=== LIABILITY DETAILS ==="]

        # Process Standard Fields
        for field in LIABILITIES_NEW_SCHEMA:
            key = field['api']
            val = record.get(key)
    ...
    def generate_response(self, user_query: str, liability_data: dict, history: list = []) -> str:
        """
        Generates a response answering questions about the Liability.
        """
        context_text = self.format_data_for_ai(liability_data)
        ...
        prompt = f"""
        You are an expert Debt Manager & Financial Advisor.
        ...
        - **Connections:** Mention linked 'Real Estate' or 'Loan' records to provide context.
        - **Balance:** Report 'Total Current Loan Balance' vs 'Liability Amount'.

        ### ACTION PROTOCOL
        <<<ACTION>>>
        {{
            "action": "create" | "update",
            "module": "Liabilities_New", 
            "record_id": "12345",
            "data": {{ 
                "Name": "REQUIRED_NAME",
