...
SPECIFIC_LIST_CONFIG = {
    ...
    "Documents_Needed_New": ["Document_Name", "Status", "Required_Date"] # Assuming subform structure based on name
}

# --- KNOWLEDGE BASE: KYC DOCUMENTS SCHEMA ---
KYC_DOCUMENTS_SCHEMA = [
    {"api": "id", "type": "text"},
    {"api": "Name", "type": "text"}, # Document Reference
    {"api": "KYC_Record_ID", "type": "autonumber"},
    ...
    # Document Details
    {"api": "Document_Type", "type": "picklist"}, # e.g. Passport, Drivers License
    {"api": "KYC_Type", "type": "picklist"}, # e.g. Individual, Entity
    {"api": "Status", "type": "picklist"}, # e.g. Verified, Pending
    {"api": "Verification_Method", "type": "picklist"},
    {"api": "Verification_Date", "type": "date"},
    {"api": "Expiry_Date", "type": "date"},
    {"api": "Locked__s", "type": "boolean"},
    
    # Subform
    {"api": "Documents_Needed_New", "type": "static_subform"},
    
    # Relationships (Lookups with Targets)
    {"api": "Client", "type": "lookup", "target": "Contacts"},
    {"api": "Household", "type": "lookup", "target": "Accounts"},
    {"api": "Risks", "type": "lookup", "target": "Risk_Profile_New"},
    {"api": "Verified_By", "type": "lookup", "target": "Leads"}, # Note: Schema says Leads, unusual but kept
    
    # Advisory Team
    {"api": "Primary_Advisor", "type": "userlookup", "target": "Users"},
    {"api": "Secondary_Advisor", "type": "userlookup", "target": "Users"},
    {"api": "KYC_Documents_Owners", "type": "multiuserlookup", "target": "Users"}
]

class KYCDocumentsNewAgent:
    def __init__(self, model):
        self.model = model
        self.schema_string = "\n".join([f"- {f['api']} ({f['type']})" for f in KYC_DOCUMENTS_SCHEMA])

    def format_data_for_ai(self, record: dict) -> str:
        """
        Parses KYC Document JSON into a clean, readable text block.
        """
        if not record: return "No KYC Document Data Available."

        # === 1. LIST MODE (Context Switch / Search Results) ===
        if "items" in record:
            items = record["items"]
            if not items: return "No KYC Documents found."
            
            lines = [f"=== FOUND {len(items)} DOCUMENTS ==="]
            for i, item in enumerate(items, 1):
                ...
                critical_fields = ["Document_Type", "Status", "Expiry_Date", "Client"]
        ...
        lines = ["=== KYC DOCUMENT DETAILS ==="]

        # Process Standard Fields
        for field in KYC_DOCUMENTS_SCHEMA:
            key = field['api']
            val = record.get(key)
            ...
            if key == "KYC_Documents_Owners" and isinstance(val, list):
                names = [u.get("name", "Unknown") for u in val if isinstance(u, dict)]
                val = ", ".join(names)

            # Formatting Subforms
            if field['type'] == "static_subform" and isinstance(val, list):
                lines.append(f"\n--- {key} (Subform) ---")
        ...
        lines.append("\n--- RELATED ACTIVITY ---")
        list_keys = [k for k, v in record.items() if isinstance(v, list) and k != "Tag" and "Documents_Needed" not in k]
    ...
    def generate_response(self, user_query: str, kyc_data: dict, history: list = []) -> str:
        """
        Generates a response answering questions about the KYC Document.
        """
        context_text = self.format_data_for_ai(kyc_data)
        ...
        prompt = f"""
        You are an expert KYC & Compliance Analyst.
        ...
        - **Expiry:** Check 'Expiry Date' to see if the document is valid.
        - **Verification:** State 'Status', 'Verification Method', and 'Verification Date'.
        - **Context:** Identify the 'Client' and 'Household'.

        ### ACTION PROTOCOL
        <<<ACTION>>>
        {{
            "action": "create" | "update",
            "module": "KYC_Documents_New", 
            "record_id": "12345",
            "data": {{ 
                "Name": "REQUIRED_DOC_REF",
                "Status": "Verified",
                "Field_Name": "Value" 
            }}
        }}
