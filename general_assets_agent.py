...
# --- KNOWLEDGE BASE: GENERAL ASSETS SCHEMA ---
GENERAL_ASSETS_SCHEMA = [
    {"api": "id", "type": "text"},
    {"api": "Name", "type": "text"}, # General Assets Name
    ...
    {"api": "Current_Market_Value", "type": "currency"},
    {"api": "Valuation_Method", "type": "picklist"},
    {"api": "Last_assessed_date", "type": "date"},
    {"api": "Asset_Type", "type": "picklist"}, # e.g. Vehicle, Collectible
    {"api": "Describe_Others", "type": "text"},
    {"api": "Locked__s", "type": "boolean"},
    
    # Specific Categories
    {"api": "Vehicle_Type", "type": "picklist"},
    {"api": "Make_an_Year", "type": "integer"},
    {"api": "Personal_Valuable_Type", "type": "picklist"},
    {"api": "Collectible_Type", "type": "picklist"},
    ...
    {"api": "Asset_Management_Team", "type": "multiuserlookup", "target": "Users"}
]

class GeneralAssetsAgent:
    def __init__(self, model):
        self.model = model
        self.schema_string = "\n".join([f"- {f['api']} ({f['type']})" for f in GENERAL_ASSETS_SCHEMA])

    def format_data_for_ai(self, record: dict) -> str:
        """
        Parses General Assets JSON into a clean, readable text block.
        """
        if not record: return "No General Asset Data Available."

        # === 1. LIST MODE (Context Switch / Search Results) ===
        if "items" in record:
            items = record["items"]
            if not items: return "No General Assets found."
            ...
            for i, item in enumerate(items, 1):
                ...
                critical_fields = ["Asset_Type", "Current_Market_Value", "Vehicle_Type", "Investment_Portfolio"]
        ...
        lines = ["=== GENERAL ASSET DETAILS ==="]

        # Process Standard Fields
        for field in GENERAL_ASSETS_SCHEMA:
            key = field['api']
            val = record.get(key)
    ...
    def generate_response(self, user_query: str, asset_data: dict, history: list = []) -> str:
        """
        Generates a response answering questions about the General Asset.
        """
        ...
        prompt = f"""
        You are an expert Asset Manager & Appraiser.
        ...
        - **Valuation:** State 'Current Market Value' and 'Last assessed date' if available.
        - **Details:** If it's a Vehicle, mention 'Vehicle Type' and 'Make an Year'.

        ### ACTION PROTOCOL
        <<<ACTION>>>
        {{
            "action": "create" | "update",
            "module": "General_Assets_New", 
            "record_id": "12345",
            "data": {{ 
                "Name": "REQUIRED_NAME",
                "Asset_Type": "Vehicle",
