...
# --- KNOWLEDGE BASE: INCOME NEW SCHEMA ---
INCOME_NEW_SCHEMA = [
    {"api": "id", "type": "text"},
    {"api": "Name", "type": "text"}, # Income Name
    ...
    {"api": "Source_Employer", "type": "text"},
    {"api": "Types_of_Income", "type": "picklist"}, # Category (e.g. Salary, Dividend)
    {"api": "Income_Type", "type": "picklist"},
    {"api": "Locked__s", "type": "boolean"},
    
    # Relationships (Lookups with Targets)
    {"api": "Income_Profile", "type": "lookup", "target": "Income_Profile_New"},
    {"api": "Real_estate", "type": "lookup", "target": "Asset_Ownership_New"},
    {"api": "Investment_Holdings", "type": "lookup", "target": "Investment_portfolios"},
    {"api": "Assets", "type": "lookup", "target": "Asset_Ownership_New"}, # General Assets
    
    # Advisory Team
    {"api": "Primary_Advisor", "type": "userlookup", "target": "Users"},
    {"api": "Secondary_Advisor", "type": "userlookup", "target": "Users"},
    {"api": "Income_Owners", "type": "multiuserlookup", "target": "Users"}
]

class IncomeNewAgent:
    def __init__(self, model):
        self.model = model
        self.schema_string = "\n".join([f"- {f['api']} ({f['type']})" for f in INCOME_NEW_SCHEMA])

    def format_data_for_ai(self, record: dict) -> str:
        """
        Parses Income Record JSON into a clean, readable text block.
        """
        if not record: return "No Income Data Available."

        # === 1. LIST MODE (Context Switch / Search Results) ===
        if "items" in record:
            items = record["items"]
            if not items: return "No Income Records found."
            
            lines = [f"=== FOUND {len(items)} INCOME RECORDS ==="]
            for i, item in enumerate(items, 1):
                ...
                critical_fields = ["Amount", "Frequency", "Source_Employer", "Income_Profile"]
        ...
        lines = ["=== INCOME RECORD DETAILS ==="]

        # Process Standard Fields
        for field in INCOME_NEW_SCHEMA:
            key = field['api']
            val = record.get(key)
            ...
            if key == "Income_Owners" and isinstance(val, list):
    ...
    def generate_response(self, user_query: str, income_data: dict, history: list = []) -> str:
        """
        Generates a response answering questions about the Income Record.
        """
        ...
        - **Source:** Identify 'Source Employer' or linked Asset ('Real estate', 'Investment Holdings').
        - **Value:** State 'Amount' and 'Frequency' clearly.
