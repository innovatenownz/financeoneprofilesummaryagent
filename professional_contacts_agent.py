...
PROF_CONTACTS_SCHEMA = [
    {"api": "id", "type": "text"},
    {"api": "Name", "type": "text"}, # Professional Contact Name
    ...
    {"api": "Professional_Type", "type": "picklist"}, # e.g. Lawyer, CPA
    ...
    {"api": "Professional_Contact_Owners", "type": "multiuserlookup", "target": "Users"}
]
...
class ProfessionalContactsAgent:
    ...
    def format_data_for_ai(self, record: dict) -> str:
        ...
        """
        if not record: return "No Professional Contact Data Available."

        # === 1. LIST MODE (Context Switch / Search Results) ===
        if "items" in record:
            items = record["items"]
            if not items: return "No Professional Contacts found."
            
            lines = [f"=== FOUND {len(items)} PROFESSIONAL CONTACTS ==="]
            for i, item in enumerate(items, 1):
                ...
                critical_fields = ["Professional_Type", "Client", "Household", "Status"]
    ...
    def generate_response(self, user_query: str, prof_data: dict, history: list = []) -> str:
        """
        ...
        prompt = f"""
        You are an expert Center of Influence (COI) Manager & CRM Assistant.
        ...
        - **Identity:** Identify the professional and their role (e.g., CPA, Lawyer).

        ### ACTION PROTOCOL
        <<<ACTION>>>
        {{
            "action": "create" | "update",
            "module": "Professional_Contacts_New", 
            "record_id": "12345",
            "data": {{ 
                "Name": "REQUIRED_NAME",
