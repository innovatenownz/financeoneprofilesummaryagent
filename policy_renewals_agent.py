...
# --- KNOWLEDGE BASE: POLICY RENEWALS SCHEMA ---
POLICY_RENEWALS_SCHEMA = [
    {"api": "id", "type": "text"},
    {"api": "Name", "type": "text"}, # Policy Renewal Name
    ...
    {"api": "Policy_Renewals_Owners", "type": "multiuserlookup", "target": "Users"}
]

class PolicyRenewalsAgent:
    def __init__(self, model):
        self.model = model
        self.schema_string = "\n".join([f"- {f['api']} ({f['type']})" for f in POLICY_RENEWALS_SCHEMA])

    def format_data_for_ai(self, record: dict) -> str:
        """
        Parses Policy Renewal JSON into a clean, readable text block.
        """
        if not record: return "No Policy Renewal Data Available."

        # === 1. LIST MODE (Context Switch / Search Results) ===
        if "items" in record:
            items = record["items"]
            if not items: return "No Policy Renewals found."
            
            lines = [f"=== FOUND {len(items)} RENEWALS ==="]
            for i, item in enumerate(items, 1):
                ...
                critical_fields = ["Status", "Insurance_Policy", "Primary_Advisor"]
        ...
        lines = ["=== POLICY RENEWAL DETAILS ==="]

        # Process Standard Fields
        for field in POLICY_RENEWALS_SCHEMA:
            key = field['api']
            val = record.get(key)
            ...
            if key == "Policy_Renewals_Owners" and isinstance(val, list):
    ...
    def generate_response(self, user_query: str, renewal_data: dict, history: list = []) -> str:
        """
        Generates a response answering questions about the Policy Renewal.
        """
        context_text = self.format_data_for_ai(renewal_data)
        ...
        prompt = f"""
        You are an expert Insurance Renewal Specialist.
        ...
        - **Context:** Identify the linked 'Insurance Policy' being renewed.
        - **Status:** Check 'Status' to determine where in the renewal lifecycle this is.

        ### ACTION PROTOCOL
        <<<ACTION>>>
        {{
            "action": "create" | "update",
            "module": "Policy_Renewals_New", 
            "record_id": "12345",
            "data": {{ 
                "Name": "REQUIRED_RENEWAL_NAME",
                "Status": "In Progress",
                "Field_Name": "Value" 
            }}
        }}
